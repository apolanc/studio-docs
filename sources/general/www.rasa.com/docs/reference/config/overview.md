# Source: https://www.rasa.com/docs/reference/config/overview
...
A minimal configuration for a [CALM](https://www.rasa.com/docs/learn/concepts/calm/) assistant looks like this:
...
## Assistant ID[​](https://www.rasa.com/docs/reference/config/overview/#assistant-id 'Direct link to Assistant ID')

The `assistant_id` key should be a unique value and allows you to distinguish multiple deployed assistants. This id is added to each event's metadata, together with the model id. See [event brokers](https://www.rasa.com/docs/reference/integrations/event-brokers/) for more information. Note that if the config file does not include this required key or the placeholder default value is not replaced, a random assistant name will be generated and added to the configuration every time you run `rasa train`.

## Recipe[​](https://www.rasa.com/docs/reference/config/overview/#recipe 'Direct link to Recipe')

The `recipe` key only needs to be modified if you want to use a [custom graph recipe](https://www.rasa.com/docs/reference/config/components/graph-recipe/). The vast majority of projects should use the default value `"default.v1"`.

## Language[​](https://www.rasa.com/docs/reference/config/overview/#language 'Direct link to Language')
...
The full set of configurable parameters is listed [here](https://www.rasa.com/docs/reference/config/components/llm-command-generators/).

All components which make use of LLMs have common configuration parameters which are listed [here](https://www.rasa.com/docs/reference/config/components/llm-configuration/)

## Policies[​](https://www.rasa.com/docs/reference/config/overview/#policies 'Direct link to Policies')

The `policies` key lists the [dialogue policies](https://www.rasa.com/docs/reference/config/policies/overview/) your assistant will use to progress the conversation.
...
The [FlowPolicy](https://www.rasa.com/docs/reference/config/policies/flow-policy/) currently doesn't have an additional configuration parameters.

## Silence Timeout Handling[​](https://www.rasa.com/docs/reference/config/overview/#silence-timeout-handling 'Direct link to Silence Timeout Handling')
...
### Global Silence Timeout[​](https://www.rasa.com/docs/reference/config/overview/#global-silence-timeout 'Direct link to Global Silence Timeout')
...
This means the assistant will wait 7 seconds (or your configured value) for a user reply before treating it as **silence** and triggering [fallback logic](https://www.rasa.com/docs/reference/config/overview/#customizing-the-assistants-response-to-silence).
...
This means that on twilio\_media\_streams channel the assistant will wait 10 seconds (or your configured value) for a user reply before treating it as **silence** and triggering [fallback logic](https://www.rasa.com/docs/reference/config/overview/#customizing-the-assistants-response-to-silence).

### Local (Per-Step) Silence Timeout[​](https://www.rasa.com/docs/reference/config/overview/#local-per-step-silence-timeout 'Direct link to Local (Per-Step) Silence Timeout')
...
### Enabling/Disabling Silence Timeouts[​](https://www.rasa.com/docs/reference/config/overview/#enablingdisabling-silence-timeouts 'Direct link to Enabling/Disabling Silence Timeouts')
...
### Customizing the Assistant's Response to Silence[​](https://www.rasa.com/docs/reference/config/overview/#customizing-the-assistants-response-to-silence 'Direct link to Customizing the Assistant's Response to Silence')
...
👉 [Learn more about patterns configuration](https://www.rasa.com/docs/reference/primitives/patterns/)

## Minimum pacing between bot messages (voice streams)[​](https://www.rasa.com/docs/reference/config/overview/#minimum-pacing-between-bot-messages-voice-streams 'Direct link to Minimum pacing between bot messages (voice streams)')
...
This applies to the same class of streaming voice connectors as [silence timeout handling](https://www.rasa.com/docs/reference/config/overview/#silence-timeout-handling).

### Regular utterances vs filler[​](https://www.rasa.com/docs/reference/config/overview/#regular-utterances-vs-filler 'Direct link to Regular utterances vs filler')
...
- [Assistant ID](https://www.rasa.com/docs/reference/config/overview/#assistant-id)
- [Recipe](https://www.rasa.com/docs/reference/config/overview/#recipe)
- [Language](https://www.rasa.com/docs/reference/config/overview/#language)
- [Pipeline](https://www.rasa.com/docs/reference/config/overview/#pipeline)
- [Policies](https://www.rasa.com/docs/reference/config/overview/#policies)
- [Silence Timeout Handling](https://www.rasa.com/docs/reference/config/overview/#silence-timeout-handling)
 - [Global Silence Timeout](https://www.rasa.com/docs/reference/config/overview/#global-silence-timeout)
 - [Local (Per-Step) Silence Timeout](https://www.rasa.com/docs/reference/config/overview/#local-per-step-silence-timeout)
 - [Enabling/Disabling Silence Timeouts](https://www.rasa.com/docs/reference/config/overview/#enablingdisabling-silence-timeouts)
 - [Customizing the Assistant's Response to Silence](https://www.rasa.com/docs/reference/config/overview/#customizing-the-assistants-response-to-silence)
- [Minimum pacing between bot messages (voice streams)](https://www.rasa.com/docs/reference/config/overview/#minimum-pacing-between-bot-messages-voice-streams)
 - [Regular utterances vs filler](https://www.rasa.com/docs/reference/config/overview/#regular-utterances-vs-filler)