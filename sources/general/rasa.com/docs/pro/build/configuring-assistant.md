...
### A2A Server — Expose Rasa as a sub-agent[​](https://rasa.com/docs/pro/build/configuring-assistant/#a2a-server--expose-rasa-as-a-sub-agent 'Direct link to A2A Server — Expose Rasa as a sub-agent')
...
This is the **inverse** of [integrating external agents via A2A](https://rasa.com/docs/pro/build/integrating-external-agents/), where Rasa is the orchestrator. Here, Rasa is the sub-agent that external systems discover and call.

Requires `start_session_after_expiry: false` in `domain.yml` and `SANIC_WORKERS=1` per replica.

👉 [How to expose Rasa as an A2A sub-agent](https://rasa.com/docs/pro/build/exposing-rasa-as-a2a-sub-agent/)

👉 [A2A Server configuration reference](https://rasa.com/docs/reference/integrations/a2a-server/)
...
- [Endpoints](https://rasa.com/docs/pro/build/configuring-assistant/#endpoints)
 ...
 - [A2A Server — Expose Rasa as a sub-agent](https://rasa.com/docs/pro/build/configuring-assistant/#a2a-server--expose-rasa-as-a-sub-agent)
